...
var host    = require('../core').data.host,
...
function initGuest () {
    ...
    stbNames.forEach(function forEachStbNames ( stbObjName ) {
        ...
        if ( !window.proxy.active && config.cache ) {
            stbObjKeys = JSON.parse(storage.getItem('proxy:eval:' + keysCode));
            console.log('proxy cache: ', keysCode);
        } else {
            stbObjKeys = window.proxy.eval(keysCode);
            storage.setItem('proxy:eval:' + keysCode, JSON.stringify(stbObjKeys));
        }
